    ...
    1
      2
        3
          ...
          2
    """
    new_leaves = [tree(i) for i in leaves]
    if is_leaf(t):
...
def add_trees(t1, t2):
    ...
    if l1 < l2:
        fewer_branches, more_branches = branches(t1), branches(t2)
    else:
        fewer_branches, more_branches = branches(t2), branches(t1)
    padding_branches = fewer_branches + [tree(0) for _ in range(len(more_branches) - len(fewer_branches))]
    return tree(label(t1) + label(t2), [add_trees(b1, b2) for b1, b2 in zip(padding_branches, more_branches)])
